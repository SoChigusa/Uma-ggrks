...
def to_dictionary(title, id, charas, choices, results, type):
    ...
    for choice, result in zip(choices, results):
        header = result.select(
            '.umamusume-event-checker__choice--result-header')
        if(header != []):  # success/failure-type evevnts
            casedict = {}
            for result_caseA in result.find_all('div', recursive=False):
                header = result_caseA.select(
                    '.umamusume-event-checker__choice--result-header')[0].text
                reslist_caseA = [res.text.removeprefix(
                    '・') for res in result_caseA.find_all('div')]
                casedict[header] = reslist_caseA
            chdict.append(dict([("Choice", choice), ("Results", casedict)]))
        else:
            reslist = [res.text.removeprefix('・')
                       for res in result.find_all('div')]
    ...
    mydict = dict([("Title", title), ("ID", id),
                  ("Event", event), ("Choices", chdict)])
    return mydict
    ...
    for card in soup.body.find_all('div', recursive=False):
        for event in card.select('.umamusume-event-checker__event'):
            ...
            if(title != []):  # omit .umamusume-event-checker__event no-event
                title = title[0].text
                id = event.select(
                    '.umamusume-event-checker__event--id')[0].text
                charas = event.select(
                    '.umamusume-event-checker__event--character')[0]
                charas = [chara.text for chara in charas.find_all('a')]
                choices = [ch.select(
                    '.umamusume-event-checker__choice--body')[0].text for ch in event.find_all('tr')]
                results = [ch.select(
                    '.umamusume-event-checker__choice--result')[0] for ch in event.find_all('tr')]
                card_dict.append(to_dictionary(
                    title, id, charas, choices, results, type))
...
# character events
for character in ['ain', 'agnst', 'agnsd', 'adm', 'inr', 'wnn', 'wk', 'eag', 'eas', 'eis', 'erk', 'ogr', 'krn', 'kwk', 'kts', 'kng', 'grs', 'gorst', 'gorsp', 'sir', 'skrt', 'skrb', 'stn', 'snb', 'sii', 'sup', 'sps', 'sma', 'siu', 'tik', 'diw', 'tmm', 'tie', 'tuk', 'tos', 'nis', 'nrtt', 'nrtb', 'nsn', 'hru', 'hsak', 'hsam', 'bwh', 'fin', 'hzk', 'mtknt', 'mtknh', 'myn', 'mrz', 'mnh', 'mhn', 'mis', 'mzra', 'mzrd', 'mzrp', 'mzrb', 'mzrm', 'mzrr', 'yen', 'ris']:
...
card_dict_all += html_to_json('html/scenario.html',
                              'json/scenario.json', {"Type": "Main Scenario"})
